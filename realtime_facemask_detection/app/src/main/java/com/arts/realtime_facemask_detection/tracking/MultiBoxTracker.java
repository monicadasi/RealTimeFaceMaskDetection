package com.arts.realtime_facemask_detection.tracking;
...
public class MultiBoxTracker {
  ...
  public synchronized void trackResults(final List<Recognition> results, final long timestamp) {
    Log.i("MultiBoxTracker", "trackResults: Processing " + results.size() + "results from " + timestamp);
  ...
  private void processResults(final List<Recognition> results) {
    ...
    for (final Recognition result : results) {
      ...
      Log.i("MultiBoxTracker", "processResults: Result! Frame: " +
              result.getLocation() + " mapped to screen:" + detectionScreenRect);

      screenRects.add(new Pair<Float, RectF>(result.getConfidence(), detectionScreenRect));

      if (detectionFrameRect.width() < MIN_SIZE || detectionFrameRect.height() < MIN_SIZE) {
        Log.w("MultiBoxTracker", "processResults: Degenerate rectangle! " + detectionFrameRect);
        continue;
      }
    ...
    if (rectsToTrack.isEmpty()) {
      Log.i("MultiBoxTracker", "processResults: Nothing to track, aborting");
      return;
    }
